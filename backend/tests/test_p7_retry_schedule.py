...
from app.p16.runtime import P16_TASK_TYPE, run_generation_task
...
def test_generic_retry_scheduler_dispatches_p15_p16_p17_runners(session_factory: sessionmaker[Session]) -> None:
    cases = [
        ("p15-task", P15_TASK_TYPE, run_storyboard_task),
        ("p16-task", P16_TASK_TYPE, run_generation_task),
        ("p17-task", P17_TASK_TYPE, run_post_task),
    ]

    for task_id, task_type, runner in cases:
        background_tasks = BackgroundTasks()
        task = SimpleNamespace(id=task_id, task_type=task_type)
        with session_factory() as db:
            _schedule_task_if_needed(background_tasks, db, task)

        assert len(background_tasks.tasks) == 1
        scheduled = background_tasks.tasks[0]
        assert scheduled.func is runner
        assert scheduled.args[1] == task_id
