...
class ReplicaStoryboardCandidate(Base):
    __tablename__ = "replica_storyboard_candidates"
...
class ReplicaTargetStoryboardRevision(Base):
    __tablename__ = "replica_target_storyboard_revisions"

    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid4()))
    project_id: Mapped[str] = mapped_column(ForeignKey("projects.id", ondelete="CASCADE"), nullable=False, index=True)
    artifact_id: Mapped[str] = mapped_column(ForeignKey("artifact_nodes.id", ondelete="CASCADE"), nullable=False, unique=True, index=True)
    candidate_id: Mapped[str] = mapped_column(ForeignKey("replica_storyboard_candidates.id", ondelete="RESTRICT"), nullable=False, index=True)
...
class ReplicaGenerationSegmentsRevision(Base):
    __tablename__ = "replica_generation_segments_revisions"
