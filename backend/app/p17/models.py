...
class ReplicaPostCandidate(Base):
    __tablename__ = "replica_post_candidates"

    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid4()))
    project_id: Mapped[str] = mapped_column(ForeignKey("projects.id", ondelete="CASCADE"), nullable=False, index=True)
    generation_selection_artifact_id: Mapped[str] = mapped_column(ForeignKey("artifact_nodes.id", ondelete="RESTRICT"), nullable=False, index=True)
    target_audio_artifact_id: Mapped[str | None] = mapped_column(ForeignKey("artifact_nodes.id", ondelete="RESTRICT"), nullable=True, index=True)
    ...
    timing_plan_artifact_id: Mapped[str | None] = mapped_column(ForeignKey("artifact_nodes.id", ondelete="RESTRICT"), nullable=True, index=True)
...
class ReplicaFinalOutputRevision(Base):
    __tablename__ = "replica_final_output_revisions"
    ...
    target_script_artifact_id: Mapped[str] = mapped_column(ForeignKey("artifact_nodes.id", ondelete="RESTRICT"), nullable=False, index=True)
    timing_plan_artifact_id: Mapped[str | None] = mapped_column(ForeignKey("artifact_nodes.id", ondelete="RESTRICT"), nullable=True, index=True)
    candidate_id: Mapped[str] = mapped_column(ForeignKey("replica_post_candidates.id", ondelete="RESTRICT"), nullable=False, index=True)
