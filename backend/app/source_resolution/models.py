...
class SourceResolutionRevision(Base):
    """Typed P9 revision payload for one formal resolution Artifact."""

    __tablename__ = "source_resolution_revisions"
    ...
    resolution_kind: Mapped[str] = mapped_column(String(24), nullable=False, index=True)
    ...
    source_shot_facts_artifact_id: Mapped[str] = mapped_column(ForeignKey("artifact_nodes.id"), nullable=False, index=True)
    shot_anchors_artifact_id: Mapped[str] = mapped_column(ForeignKey("artifact_nodes.id"), nullable=False, index=True)
    source_dialogue_artifact_id: Mapped[str | None] = mapped_column(ForeignKey("artifact_nodes.id"), nullable=True, index=True)
