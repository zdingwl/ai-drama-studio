...
from app.replica_pipeline.h3_prompting import create_h3_prompt_task, get_h3_prompts
...
def localized_storyboard_command(
    project_id: str,
...
def localized_storyboard_accept(
    project_id: str,
    candidate_id: str,
    command: PipelineReviewCommand,
    db: Session = Depends(get_db),
) -> LocalizedStoryboardRead:
...
def localized_storyboard_reject(
    project_id: str,
    candidate_id: str,
    command: PipelineReviewCommand,
    db: Session = Depends(get_db),
...
def localized_storyboard_update_shot(
    project_id: str,
...
def asset_images_regenerate_command(
    project_id: str,
    idempotency_key: Annotated[str, Header(alias="Idempotency-Key")],
    db: Session = Depends(get_db),
) -> TaskRead:
...
def asset_workspace_prompts(
    project_id: str,
    command: AssetWorkspaceSelectionCommand,
    idempotency_key: Annotated[str, Header(alias="Idempotency-Key")],
    db: Session = Depends(get_db),
) -> TaskRead:
    task = create_asset_workspace_task(
        db,
        project_id=project_id,
        idempotency_key=idempotency_key,
        operation="prompts",
        target_asset_ids=command.target_asset_ids,
    )
    return task_to_read(task)
