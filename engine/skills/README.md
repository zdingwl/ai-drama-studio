...
`CONTRACT_ONLY` 只表示契约已经定义并可以被注册表校验，**不表示已经接入生产模型调用**。现有拉片、人物、对白、TTS、H3、QC 等运行链不会因为这些文件出现而改变。

只有完成 Provider 接入、输入/输出 Validator、显式命令入口、revision/fingerprint、测试和运行记录后，Skill 才能单独升级为 `RUNTIME_READY`。

## 正式 Skill

| Skill ID | 作用 | 当前状态 |
| --- | --- | --- |
| `shot_facts` | 单镜事实拉片 | CONTRACT_ONLY |
| `episode_understanding` | 整集剧情结构理解 | CONTRACT_ONLY |
| `screenplay_reconstruction` | 从已确认原片事实还原正式剧本 | CONTRACT_ONLY |
...
1. `SourceDramaSnapshot` 是目标侧正式原片事实来源，Skill 不得绕过正式人物、场景、对白、speaker 和 revision 门禁去消费瞬态证据。
