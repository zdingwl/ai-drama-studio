...
def _seed(monkeypatch, tmp_path: Path, shots: list[dict], dialogues: list[dict]):
    ...
    with factory() as session:
        ...
        session.commit()
    source = _source(project["id"], episode_id, shots)
...
def test_keep_when_real_speech_finishes_inside_source_shot(monkeypatch, tmp_path: Path) -> None:
    ...
    project_id, _ = _seed(monkeypatch, tmp_path, shots, dialogues)
