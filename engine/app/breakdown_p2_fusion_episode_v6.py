...
class SubjectContinuityStats:
    ...
    component_bridge_union_count: int
...
def _build_subject_cluster_keys(
    ...
    component_bridge_union_count = 0
    ...
    for segment_plan in segment_plans:
        ...
        if clusters:
            try:
                ...
                pass
            try:
                component_bridge_union_count += int(clusters[0].get("accepted_component_bridge_count") or 0)
            except (TypeError, ValueError):
                pass
        ...
        for cluster in clusters:
            ...
            if not first_revision_item_id or not first_label:
                raise legacy.BreakdownP2FusionError("E6 cluster member missing revision_item_id/label")
            key = (
                f"e6:{first_revision_item_id}:{first_label}"
                if len(members) > 1
            ...
            for member in members:
                ...
                if node in mapped_nodes:
                    raise legacy.BreakdownP2FusionError("E6 anonymous observation mapped to multiple clusters")
        ...
        if mapped_nodes != expected_nodes:
            ...
            raise legacy.BreakdownP2FusionError(
                f"E6 anonymous cluster coverage mismatch: missing={missing}, extra={extra}"
            )
    ...
    return final_keys, cluster_members, SubjectContinuityStats(
        ...
        component_bridge_union_count=component_bridge_union_count,
    ...
    *,
    ...
    fusion_status.update({
        ...
        "subject_continuity": {
            ...
            "component_bridge_union_count": stats.component_bridge_union_count,
...
def fuse_breakdown_run(run_id: str) -> BreakdownRun:
    ...
    try:
        ...
        with e1._FUSION_PATCH_LOCK:
            ...
            legacy._segment_plans = e6_segment_plans
            legacy._appearance_key = e6_appearance_key
            e1._continuity_plan_details = e6_plan_details
            try:
                ...
                if stats.observation_count >= 4 and stats.merged_cluster_count == 0:
                    warnings.append({
                        "code": "E6_SUBJECT_CONTINUITY_UNRESOLVED",
                        "message": "E6 未形成任何跨镜匿名人物连续簇；请检查 Window hints / stable appearance Evidence",
                    })
                ...
                with studio_v2.get_session() as session:
                    ...
                    if run.status != "PROCESSING":
                        raise legacy.BreakdownP2FusionError(
                            f"E6 post-Fusion 只允许处理 PROCESSING Run，当前状态为 {run.status}"
                        )
                    ...
                    _rewrite_e6_metadata(session, run=run, stats=stats)
                    session.commit()
            finally:
