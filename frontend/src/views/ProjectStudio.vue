...
// 资产工作区自己的选中状态。拉片工作区已经拆成独立组件，不再与这里共用编辑状态。
...
function seconds(us: number | null | undefined) {
  if (us === null || us === undefined) return '—'
  return `${(us / 1_000_000).toFixed(2)}s`
}

function stageReady(index: number) {
  return index <= 3
}
...
function componentText(key: string) {
  ...
  const labels: Record<string, string> = {
    ...
    NOT_CONFIGURED: '未配置',
    NOT_AVAILABLE: '依赖未安装',
    MODEL_NOT_READY: '模型未准备',
    MODEL_MISSING: '模型路径不存在',
    NO_CHARACTER: '未识别人',
    NO_SCENE: '未识别场景',
    FAILED: '失败',
    BASIC: '基础描述',
...
function dragStart(episodeId: string) {
...
async function removeEpisode(episode: Episode) {
  ...
  await run('正在删除剧集', () => api.deleteEpisode(episode.id), async () => {
    if (selectedEpisodeId.value === episode.id) {
      selectedEpisodeId.value = project.value?.episodes[0]?.id ?? ''
      shots.value = []
      selectAssetShot(null)
      if (selectedEpisodeId.value) await loadAssetShots(selectedEpisodeId.value)
    }
  })
}

async function loadAssetShots(episodeId = selectedEpisodeId.value) {
  if (!episodeId) {
    shots.value = []
    selectAssetShot(null)
    return
  ...
  selectAssetShot(shots.value.find((shot) => shot.id === previous) ?? shots.value[0] ?? null)
}

async function chooseAssetEpisode(episodeId: string) {
  selectedEpisodeId.value = episodeId
  await loadAssetShots(episodeId)
}
...
<template>
  ...
  <div v-else class="studio-shell">
    <aside class="studio-sidebar">
      ...
      <nav class="stage-nav">
        <button
          ...
          :key="stage[0]"
          :class="['stage-item', { active: activeStage === index + 1 }]"
          @click="activeStage = index + 1"
        >
    ...
    <main :class="['studio-main', { 'shot-stage-main': activeStage === 2 }]">
      <header v-if="activeStage !== 2" class="workspace-header">
        <div>
          <div class="eyebrow">{{ stages[activeStage - 1][0] }} · {{ stages[activeStage - 1][1] }}</div>
          <h1>{{ stages[activeStage - 1][2] }}</h1>
        </div>
      ...
      <p v-if="activeStage !== 2 && error" class="error-banner">{{ error }}</p>
      <div v-if="activeStage !== 2 && busy" class="busy-banner"><span class="spinner"></span>{{ busy }}…</div>
      ...
      <section v-if="activeStage === 1" class="workspace-panel">
        ...
        <div v-if="episodes.length === 0" class="empty-state large">
          <strong>还没有剧集</strong>
          <span>一次选择多个视频导入即可，拉片时系统会自动准备分析素材。</span>
        </div>
        ...
        <div class="architecture-note">
          <strong>项目设置</strong>
          <p>{{ project.name }} · {{ project.source_language }} → {{ project.target_language }} · {{ project.target_region }}。项目语言和目标市场属于项目属性，不再占一个生产阶段。</p>
        </div>
      </section>
      ...
      <ShotWorkbenchV3
        ...
        :episodes="episodes"
        @refresh-project="refreshProjectFromShotWorkbench"
      />
      ...
      <section v-else-if="activeStage === 3" class="workspace-panel f05-workspace asset-workspace">
        ...
        <p class="section-help asset-help">后台从每个 Shot 提取人物 / 场景 / 道具 Evidence。页面按 Episode → Shot → Binding 组织，后续人工 Final Binding 也继续放在这里。</p>

        <div v-if="contentRun?.status === 'STALE'" class="asset-warning">
          <div>
            <strong>Shot Current Revision 已变化，当前资产结果已过期</strong>
            <span>旧 Evidence 仍保留用于对照，但重新提取后才可作为当前资产结果。</span>
          </div>
          <button class="primary-button" :disabled="!!busy" @click="analyzeContent">重新提取资产</button>
        </div>

        <div v-if="hiddenBodyOnlyCandidates > 0" class="asset-warning">
          <div>
            <strong>当前旧 Run 含 {{ hiddenBodyOnlyCandidates }} 个无 Face/SFace 身份锚点的 body-only 候选</strong>
            <span>这些候选已从正式人物列表隐藏。旧算法会把花、衣服、背景纹理等 HOG 误检当成人物。</span>
          </div>
          ...
          <div v-if="assetView === 'shot'" class="asset-manager-grid">
            <aside class="asset-episode-panel">
              ...
              <div class="asset-episode-list">
                <button v-for="episode in episodes" :key="episode.id" :class="{ active: selectedEpisodeId === episode.id }" @click="chooseAssetEpisode(episode.id)">
            ...
            <main class="asset-shot-panel">
              <div class="asset-panel-heading asset-shot-heading"><div><span>SHOT BROWSER</span><strong>{{ selectedEpisode?.title || '请选择剧集' }}</strong></div><b>{{ shots.length }} Shots</b></div>
              ...
              <div v-else class="asset-shot-list">
                <button v-for="shot in shots" :key="shot.id" :class="{ active: selectedShot?.id === shot.id }" @click="selectAssetShot(shot)">
            ...
            <aside v-if="selectedShot" class="asset-inspector">
              ...
              <video :key="`${selectedShot.id}-${selectedShot.start_us}-${selectedShot.end_us}`" :src="shotMediaUrl(selectedShot)" controls preload="metadata"></video>
      ...
      <section v-else class="workspace-panel planned-panel">
        ...
        <div class="architecture-note">
          <strong>只在高风险结果停下来</strong>
          <p>Whisper、OCR、VLM、SAM、Camera Motion 等自动能力都在后台执行；页面只开放真正需要人工决策和修改的业务结果。</p>
        </div>
      </section>
    </main>
  </div>
