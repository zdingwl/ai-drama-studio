...
async function onAdvancedChanged(): Promise<void> {
  await load()
  if (!unresolved.value.length) showAdvanced.value = false
}
...
<template>
  <section class="source-character-library">
    <template v-if="showAdvanced">
      <header class="review-mode-head">
        <button type="button" @click="showAdvanced = false">← 返回人物资产库</button>
        <div>
          <strong>人工核对原片人物</strong>
          <span>这里只处理系统无法唯一判断的人物；完成一个会自动进入下一个。</span>
        </div>
      </header>
    ...
    <template v-else>
      ...
      <section class="asset-section manual-section" :class="{ attention: unresolved.length > 0 }">
        <div class="section-title">
          <div>
            <h3>需要人工判断 {{ unresolved.length }}</h3>
            <p>系统不能唯一确认的才需要你处理。进入后每次只判断一个人物，不需要多选归并。</p>
          </div>
          <button v-if="unresolved.length" type="button" class="primary" @click="showAdvanced = true">
            开始处理 {{ unresolved.length }} 个问题 →
          </button>
        </div>
      ...
      <section v-if="suggestions.length" class="asset-section suggestion-section">
        <div class="section-title">
          <div>
            <h3>系统已经能唯一判断</h3>
            <p>这些人物在所有出现分镜中的 Final Character 交集只有一个，可以直接确认。</p>
          </div>
        </div>
      ...
      <section class="asset-section">
        <div class="section-title">
          <div>
            <h3>正式人物资产</h3>
            <p>已经确认的人物及其分镜绑定。这里主要用于查看结果，不需要重复审核。</p>
          </div>
...
<style scoped>
.source-character-library { display: grid; gap: 12px; }
.library-header, .section-title, .observation-row, .character-detail, .review-mode-head { display: flex; align-items: center; justify-content: space-between; gap: 16px; }
.library-header { padding: 16px 18px; border: 1px solid #dce4ef; border-radius: 14px; background: #fff; }
...
.review-mode-head { justify-content: flex-start; padding: 9px 11px; border: 1px solid #dfe5ed; border-radius: 10px; background: #fff; }
.review-mode-head > div { display: grid; gap: 2px; }.review-mode-head strong { color: #3b506c; font-size: 11px; }.review-mode-head span { color: #7f8b9d; font-size: 9px; }
...
.metrics article.suggested { border-color: #cbdaf8; background: #f6f9ff; }.metrics article.warning { border-color: #efcf9b; background: #fff8eb; }
.asset-section { padding: 15px; border: 1px solid #dfe5ed; border-radius: 12px; background: #fff; }.asset-section.attention { border-color: #e7ca96; background: #fffaf0; }
.section-title { margin-bottom: 12px; align-items: flex-start; }.section-title h3 { margin: 0 0 3px; color: #344a68; font-size: 14px; }
.section-title input { width: 220px; min-height: 34px; border: 1px solid #d8e0eb; border-radius: 8px; padding: 0 10px; outline: none; }.section-title input:focus { border-color: #7898d6; box-shadow: 0 0 0 3px rgba(66,111,210,.09); }
.empty, .success { padding: 18px; border-radius: 9px; background: #f7f9fc; color: #758398; font-size: 12px; text-align: center; }.success { background: #f1faf5; color: #3e785d; }
.character-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(240px, 1fr)); gap: 9px; }
.character-card { position: relative; display: grid; grid-template-columns: 62px 1fr; gap: 10px; min-height: 86px; padding: 10px; text-align: left; }.character-card:hover, .character-card.active { border-color: #8da7dc; background: #f7faff; }
.cover { width: 62px; height: 66px; display: grid; place-items: center; overflow: hidden; border-radius: 8px; background: #edf1f6; color: #667891; font-size: 24px; font-weight: 800; }.cover img { width: 100%; height: 100%; object-fit: cover; }
.character-copy { min-width: 0; display: grid; align-content: center; gap: 3px; }.character-copy > strong { color: #2f425e; font-size: 14px; }.character-copy > small { color: #8490a2; font-size: 9px; }
.character-stats { display: flex; gap: 7px; flex-wrap: wrap; }.character-stats span { padding: 2px 5px; border-radius: 5px; background: #eef3f9; color: #65758c; font-size: 9px; }.state { position: absolute; top: 8px; right: 8px; color: #4e8a69; font-size: 9px; font-weight: 800; }
.character-detail { margin-top: 10px; padding: 12px; border-radius: 10px; background: #f7f9fc; align-items: flex-start; }.character-detail h4 { margin: 3px 0; color: #30445f; }.detail-observations { min-width: min(460px, 50%); display: grid; gap: 3px; color: #6f7e91; font-size: 10px; }.detail-observations strong { color: #485a72; }
.suggestion-section { border-color: #d7e2f7; }.observation-list { display: grid; gap: 7px; }.observation-row { padding: 8px; border: 1px solid #e0e6ef; border-radius: 9px; background: #fff; }
.observation-row > img { width: 74px; height: 52px; border-radius: 7px; object-fit: cover; background: #edf1f5; }.observation-row > div:nth-child(2) { min-width: 0; flex: 1; display: grid; gap: 2px; }.observation-row > div:nth-child(2) strong { color: #354861; font-size: 12px; }.observation-row > div:nth-child(2) span, .observation-row > div:nth-child(2) small { color: #7f8b9c; font-size: 10px; }
.suggestion-target { min-width: 130px; display: grid; gap: 2px; }.suggestion-target small { color: #8b96a5; font-size: 9px; }.suggestion-target strong { color: #3f6298; font-size: 11px; }
.unresolved-preview { display: flex; flex-wrap: wrap; gap: 6px; }.unresolved-preview span, .unresolved-preview small { padding: 5px 7px; border-radius: 6px; background: #fff4e2; color: #8a6a36; font-size: 10px; }
