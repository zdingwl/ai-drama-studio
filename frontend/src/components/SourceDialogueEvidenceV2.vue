...
type Evidence = { id: string; revision: string; status: string; start_us: number; end_us: number; asr_text?: string; subtitle_text: string; utterance_id?: string }
...
async function load() {
  ...
  try {
    ...
    if (token === serial) rows.value = data
...
async function decide(row: Evidence, choice: string) {
  ...
  try {
    const response = await fetch(`/api/episodes/${encodeURIComponent(props.episodeId)}/dialogue-evidence/${row.id}/decide`, {
      method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({revision: row.revision, choice}),
    })
...
<template>
  <section v-if="visible.length || error" class="dialogue-evidence" aria-label="语音与字幕校正">
    ...
    <article v-for="row in visible" :key="row.id">
      ...
      <div><button type="button" :disabled="busy || disabled" @click="decide(row,'SUBTITLE')">采用字幕为对白</button>
