...
const taskStageText = computed(() => {
  ...
  if (task.progress_percent < 10) return '正在通读各集剧情，统一规划目标地区的人物、家庭关系、场景和道具。'
...
async function refreshTask(silent = false) {
  try {
    ...
    if (taskRunning.value) {
      ...
      stopTaskPolling()
      if (message.value === '本土化分镜任务已启动，可在下方查看实时进度。') message.value = ''
...
async function persistAutoSave(shotId: string, snapshot: typeof editDraft.value, fingerprint: string) {
  ...
  try {
    const saved = await updateLocalizedStoryboardShot(projectId.value, {
      ...
      expected_candidate_fingerprint: pending.value?.input_fingerprint ?? null,
...
<template>
  <section class="workspace" data-testid="localized-storyboard-workspace">
    ...
    <details v-if="content" class="world-plan">
      <summary>人物与场景设定 <span>{{ content.world_design_zh ? '统一规划' : '历史版本：未记录整体规划' }}</span></summary>
      <p v-if="content.world_design_zh">{{ content.world_design_zh }}</p>
      <ul v-if="content.continuity_rules_zh?.length"><li v-for="rule in content.continuity_rules_zh" :key="rule">{{ rule }}</li></ul>
      <div class="world-entities">
        <article v-for="person in content.characters" :key="person.target_character_id"><b>{{ person.display_name }}</b><p>{{ person.identity_description_zh }}</p><p>{{ person.appearance_description_zh }}</p></article>
        <article v-for="scene in content.scenes" :key="scene.target_scene_id"><b>{{ scene.display_name }}</b><p>{{ scene.setting_description_zh }}</p><p>{{ scene.visual_description_zh }}</p></article>
        <article v-for="prop in content.props" :key="prop.target_prop_id"><b>{{ prop.display_name }}</b><p>{{ prop.function_description_zh }}</p><p>{{ prop.visual_description_zh }}</p></article>
      </div>
    </details>
      ...
      <div class="review-shell">
        <aside class="shot-browser" aria-label="本土化镜头列表">
          ...
          <button v-for="shot in filteredShots" :key="shot.storyboard_shot_id" type="button" class="shot-row" :class="{ active: selectedShot?.storyboard_shot_id === shot.storyboard_shot_id }" @click="selectShot(shot.storyboard_shot_id)">
            ...
            <span class="row-copy"><strong>第 {{ shot.episode_order }} 集 · Shot {{ shot.shot_number }}</strong><small>{{ shot.dialogue[0]?.target_dialogue_zh || shot.localized_visual_description_zh }}</small><em>目标 {{ seconds(shot.duration_us) }} · 原片 {{ seconds(shot.source_duration_us ?? shot.duration_us) }} · {{ shot.output_ratio }}</em></span>
        ...
        <article v-if="selectedShot" class="shot-inspector">
          <div class="shot-head"><div><span>本土化镜头</span><strong>第 {{ selectedShot.episode_order }} 集 · Shot {{ selectedShot.shot_number }}</strong></div><div class="shot-head-actions"><small>目标 {{ seconds(selectedShot.duration_us) }} · 原片 {{ seconds(selectedShot.source_duration_us ?? selectedShot.duration_us) }} · {{ selectedShot.output_ratio }}</small><em :class="{ saving: autoSaving }">{{ autoSaving ? '正在自动保存…' : '点击内容即可修改' }}</em></div></div>
          <div class="fact-grid">
...
<style scoped>
.world-plan{padding:12px 14px;border:1px solid #e5e2ee;border-radius:10px;background:#faf9ff;font-size:13px;line-height:1.7}.world-plan summary{cursor:pointer;font-weight:700}.world-plan summary span{margin-left:10px;color:#777;font-size:12px;font-weight:400}.world-entities{display:grid;grid-template-columns:repeat(auto-fit,minmax(260px,1fr));gap:10px;max-height:440px;overflow:auto}.world-entities article{padding:12px;background:#fff;border:1px solid #eee;border-radius:8px}.world-entities p{margin:6px 0}
