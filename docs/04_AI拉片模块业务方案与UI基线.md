...
2026-09-07 确认交互调整：拉片页提供“本集人物连续确认”“本集对白连续确认”弹窗入口。每次只展示一个待确认项，保存成功自动展示下一项；原片视频、人物证据及关联分镜作为审核上下文，用户无需逐个查找分镜。人物分组、镜头资产与出镜区域保留辅助入口。对白先核对语音／字幕冲突，再明确开始说话人审核，阻塞原因直接呈现在弹窗中。
...
本文的布局与业务目标已进入代码实现和本地验证；当前完成情况与限制以 `03_当前项目状态与验收.md` 为准。
...
本文件中的目标流程不等于全部已经落地。尤其首次无整集基线的单镜分析、修改边界后的局部失效重建，仍需后续实现和独立验收。
